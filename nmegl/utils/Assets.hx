...
import format.XFL;
	...
	//todo
class Font { public function new(a){} }
class Sound { public function new(a){} }
...
class Assets {
	 ...
	 * @return		A new BItmapData object
		...
		if (AssetData.type.exists(id) && AssetData.type.get(id) == IMAGE) {
			...
			if (AssetData.library.exists(libraryName)) {
				...
				trace("[nmegl.utils.Assets] There is no asset library named \"" + libraryName + "\"");
				
			}
			
		} else {
			
			trace("[nmegl.utils.Assets] There is no BitmapData asset with an ID of \"" + id + "\"");
	...
	public static function getBytes(id:String):ByteArray {
		...
		if (AssetData.type.exists(id)) {
			...
			trace("[nmegl.utils.Assets] There is no String or ByteArray asset with an ID of \"" + id + "\"");
	...
	public static function getFont(id:String):Font {
		...
		if (AssetData.type.exists(id) && AssetData.type.get(id) == FONT) {
			...
			trace("[nmegl.utils.Assets] There is no Font asset with an ID of \"" + id + "\"");
	...
	public static function getSound(id:String):Sound {
		...
		trace("[nmegl.utils.Assets] There is no Sound asset with an ID of \"" + id + "\"");
...
class Assets {
	...
	macro public static function embedFont ():Array<Field> {
		...
		if (path != null && path != "") {
			...
			#if html5
			Sys.command ("haxelib", [ "run", "nme", "generate", "-font-hash", sys.FileSystem.fullPath(path) ]);
			path += ".hash";
			#end
