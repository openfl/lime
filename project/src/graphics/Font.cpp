...
namespace lime {
	...
	value Font::LoadGlyphs (int size, const char *glyphs, ImageBuffer *imageBuffer) {
		...
		for (std::list<GlyphInfo>::iterator it = glyphList.begin(); it != glyphList.end(); it++) {
			...
			if (x + bitmap.width > imageBuffer->width) {
			...
			if (y + bitmap.rows > imageBuffer->height) {
				...
				imageBuffer->Resize (imageBuffer->width, imageBuffer->height, 1);
				rectsIndex = 0;
				it = glyphList.begin ();
				it--;
				x = y = maxRows = 0;
			...
			if (imageBuffer->bpp == 1) {
				
				imageBuffer->Blit (bitmap.buffer, x, y, bitmap.width, bitmap.rows);
				
			} else {
				
				for (int row = 0; row < bitmap.rows; row++) {
					
					unsigned char *out = &bytes[((row + y) * imageBuffer->width + x) * imageBuffer->bpp];
