...
class NativeWindow {
	...
	public function create (application:Application):Void {
		...
		if (parent.config != null) {
			...
			if (Reflect.hasField (parent.config, "highdpi") && parent.config.highdpi) {
				if (!Reflect.hasField (parent.config, "fullscreen") || !parent.config.fullscreen) {
					flags |= cast WindowFlags.WINDOW_FLAG_HIGH_DPI;
				}
			}
...
@:enum private abstract WindowFlags(Int) {
	...
	var WINDOW_FLAG_HIGH_DPI = 0x00000800;
